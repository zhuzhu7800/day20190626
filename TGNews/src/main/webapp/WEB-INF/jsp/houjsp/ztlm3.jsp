...
<script>
	...
	function show() {
		$("input[name=tname]").val(decodeURI(window.location.search.substr(1)));
		$.ajax({
...
<body>
	...
	<div class="tcbox width500" id="tcbox1" style="display: none">
		...
		<form class="tc_content" action="/TGNews/newsss/add" method="post">
			<p class="tc_p01">
				<span class="tc_span02">栏目名称:</span> <input type="text"
					class="input01 width300" value="" name="cname">
			</p>
			<p class="tc_p01">
				<span class="tc_span02">所属专题:</span> <input type="text"
					class="input01 width300" value="" disabled='disabled' name="tname">
			</p>
			<div class="tc_btn">
	...
	<div class="tcbox width500" id="tcbox2" style="display: none">
		...
		<form class="tc_content" action="/TGNews/newsss/upd" method="post">
...
function TGDialogS(e,id){
	...
	function toUpd(id) {
		$("input[name=tname]").val(decodeURI(window.location.search.substr(1)));
		$.ajax({
			"url":"/TGNews/newsss/updTo/"+id,
			"dataType":"json",
			"type":"post",
			"success":function(result){
				$("input[name=id]").val(result.id);
				$("input[name=cname]").val(result.cname);
			},
		});
	}
}
